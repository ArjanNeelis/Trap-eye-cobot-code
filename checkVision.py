...
def capture_photo():
    cap = cv2.VideoCapture(2) #2 voor relasense
    ret, frame = cap.read()
    cap.release()
...
def HVS():
    ...
    cv2.createTrackbar('HueMin', 'Trackbars', 0, 179, empty)    #hier vul je de waardes in van de kleur die je wilt herkennen
...
def pixel(p1,p2,p3,p4):
    ...
    if blackPix1 > 10 and blackPix1 < 4500 and blackPix2 > 10 and blackPix2 < 4500:
        check = True
    else:
        check = False
